...
class KingdomBuilderBoard extends APP_GameClass
{
  ...
  public function getLocations()
  {
    ...
    for ($x = 0; $x < 20; $x++) {
      for ($y = 0; $y < 20; $y++) {
        if ($board[$x][$y] >= HEX_ORACLE || $board[$x][$y] == HEX_CASTLE) {
...
##################*/
  ...
  {
    return array_values(
      array_uintersect($this->getNeighbours($space), $positions, ['KingdomBuilderBoard', 'compareCoords'])
    );
  }
  ...
  public function keepAdjacentIfPossible(&$hexes, $pId)
  {
    $hexesNeighbouring = array_values(
      array_uintersect($hexes, $this->game->board->getPlacedSettlementsNeighbouringSpaces($pId), [
        'KingdomBuilderBoard',
        'compareCoords',
      ])
    );

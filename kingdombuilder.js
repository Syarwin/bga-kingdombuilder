...
define(['dojo', 'dojo/_base/declare', 'ebg/core/gamegui', 'ebg/counter'], function (dojo, declare) {
  ...
  return declare('bgagame.kingdombuilder', ebg.core.gamegui, {
    ...
    onUpdateActionButtons: function (stateName, args, suppressTimers) {
      ...
      if (args && args.skippable) this.addActionButton('buttonSkip', _('Skip'), 'onClickSkip', null, false, 'gray');
    ...
    onClickCell: function (x, y) {
      if (!this.isCurrentPlayerActive() || !$('cell-' + x + '-' + y).classList.contains('selectable')) return;
    ...
    notif_obtainTile: function (n) {
      ...
      this.slideDestroy(tile, container, 1000, 0).then(() => this.addTile(n.args));
    },
    ...
    onClickSelectTile: function (tile) {
      if (!this.isCurrentPlayerActive()) return;
      if (!this.gamedatas.gamestate.args) return;
    ...
    makeCellSelectable: function (hexes) {
      hexes.forEach(function (pos) {
        if (!$('cell-' + pos.x + '-' + pos.y)) {
          console.error('Cell does not exists', pos);
        } else {

...
define([
  ...
  function override_addMoveToLog(logId, moveId) {
    ...
    if (
      this.gamedatas.cancelMoveIds &&
      this.gamedatas.cancelMoveIds.includes(+moveId)
    ) {
      debug(
        "Cancel notification message for move ID " +
          moveId +
          ", log ID " +
          logId
      );
  ...
  return declare("bgagame.kingdombuilder", ebg.core.gamegui, {
    ...
    setup(gamedatas) {
      ...
      gamedatas.fplayers.forEach((player) => {
        dojo.place(
          this.format_block("jstpl_player_panel", player),
      ...
      dojo.place(
        ...
        "player_name_" + gamedatas.firstPlayer
      );
      ...
      gamedatas.objectives.forEach((objective) => {
        ...
        var div = dojo.place(
          this.format_block("jstpl_objective", objective),
          "objectives"
        );
    ...
    setupBoard: function (board, firstInit) {
      ...
      board.locations.forEach((location) => {
        ...
        if (firstInit) {
          dojo.place(
            this.format_block("jstpl_tile_container", location),
            "cell-container-" + location.x + "-" + location.y
          );
        } else {
          dojo
            .query(
              "#tile-container-" + location.x + "-" + location.y + " .tile"
            )
        ...
        if (l)
          this.addTooltipHtml(
            "cell-container-" + location.x + "-" + location.y,
            this.format_block("jstpl_tilePrompt", l)
          );
      });

      board.tiles.forEach((tile) => {
        dojo.place(
          this.format_block("jstpl_tile", tile),
          "tile-container-" + tile.x + "-" + tile.y
        );
      });
    },
    ...
    setupPreference: function () {
      ...
      var updatePreference = () => {
        var value =
          preferenceSelect.options[preferenceSelect.selectedIndex].value;
        if (value == 2) dojo.addClass("board", "no-hex-border");
        else dojo.removeClass("board", "no-hex-border");
      };
    ...
    notif_showTerrain: function (n) {
      ...
      $("player-terrain-" + n.args.pId).className =
        "player-terrain terrain-" + n.args.terrain;
    },

    notif_enableTiles: function (n) {
      debug("Notif: update player tiles", n.args);
      dojo
        .query("#player-tiles-" + n.args.pId + " .tile")
        .removeClass("pending");
    },
    ...
    onEnteringState: function (stateName, args) {
      ...
      if (
        ...
        args.args &&
        args.args.tileName != "" &&
        this.gamedatas.gamestate.descriptiontile
      ) {
        this.gamedatas.gamestate.description =
          this.gamedatas.gamestate.descriptiontile;
        this.gamedatas.gamestate.descriptionmyturn =
          this.gamedatas.gamestate.descriptiontilemyturn;
    ...
    takeAction: function (action, data, callback) {
      ...
      this.ajaxcall(
        "/kingdombuilder/kingdombuilder/" + action + ".html",
        data,
        this,
        callback
      );
    ...
    notif_cancel: function (n) {
      ...
      n.args.fplayers.forEach((player) => {
        $("player-settlements-" + player.id).firstChild.innerHTML =
          player.settlements;
    ...
    startActionTimer: function (buttonId) {
      ...
      this.actionTimerFunction = () => {
        ...
        if (button == null) {
          ...
          button.innerHTML =
            this.actionTimerLabel + " (" + this.actionTimerSeconds + ")";
        } else {
    ...
    onClickCell: function (x, y) {
      if (
        ...
        !$("cell-" + x + "-" + y).classList.contains("selectable")
      )
        return;
      ...
      if (state == "playerBuild") {
        this.takeAction("build", data);
      }
      if (state == "playerMove") {
        this.onClickCellPlayerMove(data);
      }
    },
    ...
    notif_build: function (n) {
      ...
      this.slideTemporary(
        "jstpl_settlement",
        { no: this.getPlayerNo(n.args.player_id) },
        container,
        container,
        target,
        1000,
    ...
    notif_obtainTile: function (n) {
      ...
      var container = "player-tiles-" + n.args.player_id,
        tile = dojo.query(
          "#tile-container-" + n.args.x + "-" + n.args.y + " .tile"
        )[0];

      this.slideDestroy(tile, container, 1000, 0).then(() =>
        this.addTile(n.args)
      );
    },
    ...
    onClickUseTile: function () {
      ...
      this.gamedatas.gamestate.args.tiles.forEach((tile) => {
        var div = dojo.place(
          this.format_block("jstpl_tilePrompt", this.getLocation(tile)),
    ...
    onClickSelectTile: function (tile) {
      ...
      var test = this.gamedatas.gamestate.args.tiles.find(function (t) {
        return t.id == tile.id;
      });
      if (test) this.takeAction("useTile", { tileId: tile.id });
    },
    ...
    notif_argPlayerMoveTarget: function (n) {
      ...
      this.removeActionButtons();
      this.addActionButton(
        "buttonCancelMoveSelect",
        _("Cancel"),
        "onClickCancelSelectedHex",
        null,
        false,
        "gray"
      );
    ...
    notif_updateScores(n) {
      ...
      for (var pId in n.args.scores) {
        if (n.args.scores[pId] == 0) this.scoreCtrl[pId].setValue(0);
        else this.scoreCtrl[pId].toValue(n.args.scores[pId]);
      }
    },
    ...
    notif_scoringEnd: function (n) {
      ...
      if ($("objective-" + n.args.objectiveId)) {
        ...
        this.displayScoring(
          "objective-mask-" + n.args.objectiveId,
          this.gamedatas.players[n.args.playerId].color,
          n.args.total,
          1900
        );
      }
    ...
    getPlayerNo: function (playerId) {
      return this.gamedatas.fplayers.reduce(function (carry, player) {
        return player.id == playerId ? player.cno : carry;
      }, 0);
    },
